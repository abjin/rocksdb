...
3. **컴팩션 통계**:
    ...
    - **컴팩션 횟수**:
        - Bloom: **784회**
        - Ribbon: **764회**
        - → Ribbon이 약 **2.5% 더 적은 컴팩션** 발생
    
    - **컴팩션 CPU 시간**:
        - Bloom: **884,532,930 μs**
        - Ribbon: **892,376,951 μs**
        - → 평균 컴팩션 시간은 Ribbon이 약간 더 길지만 전체 횟수는 적음

4. **DB 크기 비교**:
   
    - Bloom 필터 DB: **960MB**
    - Ribbon 필터 DB: **970MB**
    - → 최종 DB 크기는 거의 동일하며, **약 1%** 내외의 차이만 존재
...
1. **성능 영향**: Ribbon 필터는 성능(ops/sec) 측면에서 Bloom 필터와 **거의 동일**한 성능을 보여줌. 쓰기/읽기 처리량 차이는 1% 미만으로 통계적으로 유의미하지 않은 수준

2. **메모리 효율성**: Ribbon 필터가 실행 중 **최대 40MB 더 적은 메모리**를 사용하여 메모리 효율성 측면에서 장점을 보임

3. **CPU 사용**: Ribbon 필터는 쓰기 작업(fillrandom) 중 **약간 더 높은 CPU 사용률**을 보이지만, 읽기 작업에서는 Bloom과 동일한 CPU 사용률을 보임

4. **컴팩션 효율성**: Ribbon 필터가 약간 더 적은 컴팩션 횟수를 기록하여 장기적으로 시스템 부하 감소 가능성 있음

5. **최종 결론**: Ribbon 필터는 **성능 저하 없이 메모리 사용량을 절약**할 수 있는 효율적인 옵션으로, 특히 메모리 제약이 있는 환경에서 Bloom 필터의 적절한 대체제가 될 수 있음
